...
func TestSet(t *testing.T) {
	...
	m.Add(2)
	m.Reserve(1)
	assert.Equal(t, m.Length(), 2, "size equals to 2")
	m.Reserve(3)
	assert.Equal(t, m.Length(), 2, "size equals to 2")
	...
	assert.Equal(t, mm.Length(), 0, "size equal to 0")

	mmm := NewSetWithCapacity[bool](-1)
	assert.Equal(t, mmm.Length(), 0, "size equal to 0")
